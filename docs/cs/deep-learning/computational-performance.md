...
flowchart TB
  subgraph DataParallel["Data parallel training"]
    direction LR
    DBatch["Global batch (N, ...)"] --> Shard["Split batch into shards"]
  ...
  subgraph ModelParallel["Layer/model parallelism"]
    direction LR
    MIn["Activation (N, d)"] --> L0["Device 0: early layers"]
