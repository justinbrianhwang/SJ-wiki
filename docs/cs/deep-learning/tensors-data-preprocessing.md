...
flowchart TB
  ...
  TrainStats --> Num["Numeric path: impute -> standardize -> float tensor (N, d_num)"]
  TrainStats --> Cat["Categorical path: one-hot or embedding ids -> (N, d_cat)"]
  TrainStats --> Seq["Sequence/image path: pad, crop, normalize -> (N, T) or (N, C, H, W)"]
