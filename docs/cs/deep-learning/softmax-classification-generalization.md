...
```mermaid
flowchart TB
  X["Input batch X: #lsqb;N, d"]"] --> Affine["Affine classifier: logits O = XW + b -> #lsqb;N, K"]"]
  Affine --> Stable["Stable log-softmax: subtract row max before exponentials"]
  Stable --> Prob["Probabilities p_hat: #lsqb;N, K"], rows sum to 1"]
  Y["True class ids y: #lsqb;N"]"] --> Gather["Gather log probability of true class"]
  Prob --> Gather
