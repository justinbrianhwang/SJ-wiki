...
```mermaid
flowchart TB
  X["Input batch X: #lsqb;N, d_in"]"] --> L1["Linear 1: W1 #lsqb;d_in, h1"] + b1 -> ["N, h1"]"]
  L1 --> A1["Activation phi, e.g. ReLU -> #lsqb;N, h1"]"]
  A1 --> D1["Dropout train only: mask m1 ~ Bernoulli(q), h1*m1/q"]
  D1 --> L2["Linear 2: W2 #lsqb;h1, h2"] + b2 -> ["N, h2"]"]
  L2 --> A2["Activation phi -> #lsqb;N, h2"]"]
  A2 --> D2["Dropout train only: mask m2 ~ Bernoulli(q)"]
  D2 --> L3["Output linear: W3 #lsqb;h2, K"] + b3 -> logits ["N, K"]"]
