...
flowchart TB
  ...
  Fwd --> Concat["Per-token concat #lsqb;h_t^f; h_t^b"] -> ["N, T, 2*d_h"]"]
  Bwd --> Concat
...
```mermaid
flowchart TB
  Xs["Input embeddings: #lsqb;N, T, d_x"]"] --> L1["LSTM layer 1 over time -> #lsqb;N, T, d_h"]"]
  L1 --> Drop1["Dropout between recurrent layers"]
  Drop1 --> L2["LSTM layer 2 over time -> #lsqb;N, T, d_h"]"]
  L2 --> Drop2["Dropout between recurrent layers"]
  Drop2 --> L3["LSTM layer 3 over time -> #lsqb;N, T, d_h"]"]
...
flowchart TB
  ...
  HprojPrev["Projected recurrent state h'_{t-1}: #lsqb;N, r"]"] --> GatesR
  GatesR --> CellR["Internal cell update C_t and hidden h_t: #lsqb;N, n"]"]
  CellR --> Project["Projection W_r h_t -> h'_t: #lsqb;N, r"], with r much smaller than n"]
