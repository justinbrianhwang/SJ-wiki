...
```mermaid
flowchart TB
  CPU["CPU issues logical address: page p + offset d"] --> Split["MMU splits address into VPN p and page offset d"]
  Split --> TLB{"Associative TLB hit for ASID, VPN p?"}

  TLB -- "hit" --> CheckPerm["Check PTE bits: valid, user, R/W/X"]
  TLB -- "miss" --> L1["Level-1 page-table index"]
  L1 --> L2["Level-2 page-table index"]
  L2 --> L3["Leaf PTE: frame number f plus protection bits"]
  L3 --> Refill["Refill TLB entry for ASID, VPN p -> PFN f"]
  Refill --> CheckPerm

  CheckPerm -- "allowed" --> Combine["Physical address = PFN f concatenated with offset d"]
  Combine --> Cache["Cache hierarchy lookup"]
  Cache --> DRAM["DRAM frame f; offset d"]
  CheckPerm -- "invalid or permission fault" --> Trap["Trap to kernel fault handler"]

  subgraph AddressSpace["Example process virtual address space"]
    direction TB
    Kernel["Kernel space: supervisor mappings"]
    Stack["User stack: grows downward"]
    Mmap["Shared libraries and memory-mapped files"]
    Heap["Heap: grows upward"]
    Data["Data and BSS"]
    Text["Text/code: executable, often read-only"]
  end

  Split -. "VPN selects one page-sized region" .-> AddressSpace
  Trap -. "kernel may update page table or signal process" .-> L3
```

This address-translation diagram makes the I/O contract explicit: the CPU supplies a virtual page number and offset, while the MMU returns a physical frame number plus the unchanged offset. The TLB hit path avoids the multi-level page-table walk; the miss path walks page-table levels, checks protection bits, and refills the TLB. The side address-space layout shows why the same translation mechanism enforces user/kernel isolation, code permissions, heap/stack growth, and shared mappings.
