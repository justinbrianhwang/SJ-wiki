...
flowchart TB
  subgraph Inputs["Synchronized calibrated inputs"]
    direction TB
    Cam["Images: #lsqb;B, N_cam, 3, H, W"]"]
    Lidar["LiDAR points: #lsqb;B, N_pts, x, y, z, intensity"]"]
    Radar["Radar detections: #lsqb;B, N_radar, range, azimuth, Doppler"]"]
  ...
  subgraph Mid["Mid-level BEV fusion path"]
    ...
    Depth["Depth distribution per pixel: #lsqb;D, H/stride, W/stride"]"]
    Lift["Lift-splat to BEV: #lsqb;C_img, X, Y"]"]
    Voxel["Voxel/pillar encoder: #lsqb;C_lidar, X, Y"]"]
    RadarFeat["Radar BEV encoder: #lsqb;C_radar, X, Y"]"]
    MapFeat["Vector map encoder: polyline attention -> #lsqb;C_map, X, Y"]"]
  ...
  Fuse --> Heads
  TrackFilter --> Heads
  Heads --> Objects["Objects and tracks: #lsqb;class, box, velocity, covariance"]"]
  Heads --> Occupancy["Occupancy / freespace grid: #lsqb;X, Y, p_occ"]"]
