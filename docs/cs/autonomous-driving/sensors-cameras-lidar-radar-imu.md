...
flowchart TB
  ...
  subgraph Lidar["LiDAR processing chain"]
    direction TB
    LidarRaw["Packets/range image: (beam, azimuth, range, intensity, t)"]
    LidarMotion["Motion compensation with IMU/odometry"]
    LidarCloud["Point cloud: (N, x, y, z, intensity)"]
    LidarVoxel["Voxel/pillarization: (C, X, Y, Z) or (C, X, Y)"]
  ...
  subgraph Radar["Radar processing chain"]
    direction TB
    RadarRaw["FMCW ADC samples: (chirp, antenna, sample)"]
  ...
  subgraph Inertial["IMU, GNSS, odometry chain"]
    direction TB
    IMURaw["IMU: (a_x, a_y, a_z, gyro_x, gyro_y, gyro_z) at high rate"]
