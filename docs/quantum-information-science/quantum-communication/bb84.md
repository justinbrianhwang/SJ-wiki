...
![BB84 network setup showing Alice sending polarized light through an insecure quantum channel to Bob while Eve may eavesdrop](https://upload.wikimedia.org/wikipedia/commons/thumb/1/1c/BB84-network_setup.svg/640px-BB84-network_setup.svg.png)

*Figure: BB84 prepare-and-measure channel with an eavesdropper model. Image: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:BB84-network_setup.svg), Andy Spencer, CC BY-SA 3.0.*
...
sequenceDiagram
  ...
  participant C as Auth classical channel
  participant E as Eve model

  A->>Q: BB84 states from bits and basis b
  E-->>Q: possible interaction
  Q->>B: noisy received qubits
  B->>B: random Z or X measurement
  B->>C: reception notice
  A->>C: announce basis string b
  A->>C: choose check positions
  B->>C: reveal check outcomes
  C-->>A: estimate QBER and abort if high
  C-->>B: estimate QBER and abort if high
  A->>C: reconciliation data using C1
  C-->>B: correct to codeword vk
  A->>C: public hash or coset rule using C2
  C-->>A: final key
  C-->>B: final key
```
