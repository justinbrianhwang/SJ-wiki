...
Electric Potential and Capacitance covers electric potential, potential energy, equipotentials, capacitors, and stored field energy. The goal is to recognize the physical structure before choosing equations. In the HRW progression, this topic extends earlier mechanics and prepares later field, wave, or modern-physics reasoning by reusing the same habits: define the system, choose signs and variables, state assumptions, and check dimensions.
...
- **Primary symbols:** $V$, $U$, $C$, $Q$, $d$, $A$.
- **Reference equations:** $V=U/q$, $V=kq/r$, $\Delta V=-\int\vec E\cdot d\vec s$, $C=Q/\Delta V$, $U_C=\frac12C(\Delta V)^2$.
...
$$
\begin{aligned}
\text{core formulas} &: $V=U/q$, $V=kq/r$, $\Delta V=-\int\vec E\cdot d\vec s$, $C=Q/\Delta V$, $U_C=\frac12C(\Delta V)^2$.
\end{aligned}
$$
...
|---|---|---|
| Main model | electric potential, potential energy, equipotentials, capacitors, and stored field energy | assumptions must match the formula |
| Symbols | $V$, $U$, $C$, $Q$, $d$, $A$ | define each before use |
| Key formulas | $V=U/q$, $V=kq/r$, $\Delta V=-\int\vec E\cdot d\vec s$, $C=Q/\Delta V$, $U_C=\frac12C(\Delta V)^2$ | check dimensions term by term |
| Limiting case | simplify one parameter | result should match intuition |

## Worked example 1: Direct calculation

**Problem.** A point is $0.20\,\mathrm{m}$ from $+5.0\,\mathrm{nC}$ and $0.30\,\mathrm{m}$ from $-2.0\,\mathrm{nC}$. Find potential.
...
$$
$V=k(q_1/r_1+q_2/r_2)=8.99\times10^9(5.0\times10^{-9}/0.20-2.0\times10^{-9}/0.30)=165\,\mathrm{V}$..
$$
...
**Problem.** A $10\,\mu\mathrm{F}$ capacitor is connected to $9.0\,\mathrm{V}$. Find charge and stored energy.
...
$$
$Q=CV=(10\times10^{-6})(9.0)=90\,\mu\mathrm{C}$ and $U=\frac12CV^2=4.05\times10^{-4}\,\mathrm{J}$..
$$
...
```python
k=8.99e9
V=k*(5e-9/0.20-2e-9/0.30)
C=10e-6; volt=9.0
print(V,C*volt,0.5*C*volt**2)
```

## Common pitfalls

- Confusing potential with potential energy.
- Forgetting signs in scalar potential sums.
- Adding fields as scalars but potentials as vectors.
- Using capacitor series and parallel rules backward.
