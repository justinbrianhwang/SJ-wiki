---
title: Rotation, Torque, and Angular Momentum
sidebar_position: 8
---

# Rotation, Torque, and Angular Momentum

Rotation, Torque, and Angular Momentum covers rotation of rigid bodies, torque, rolling, and angular momentum. The goal is to recognize the physical structure before choosing equations. In the HRW progression, this topic extends earlier mechanics and prepares later field, wave, or modern-physics reasoning by reusing the same habits: define the system, choose signs and variables, state assumptions, and check dimensions.
...
- **Primary symbols:** $\theta$, $\omega$, $\alpha$, $I$, $\tau$, $L$.
- **Reference equations:** $\tau=rF\sin\theta$, $\sum\tau=I\alpha$, $K=\frac12I\omega^2$, $L=I\omega$, $v=R\omega$.
...
$$
\begin{aligned}
\text{core formulas} &: $\tau=rF\sin\theta$, $\sum\tau=I\alpha$, $K=\frac12I\omega^2$, $L=I\omega$, $v=R\omega$.
\end{aligned}
$$
...
|---|---|---|
| Main model | rotation of rigid bodies, torque, rolling, and angular momentum | assumptions must match the formula |
| Symbols | $\theta$, $\omega$, $\alpha$, $I$, $\tau$, $L$ | define each before use |
| Key formulas | $\tau=rF\sin\theta$, $\sum\tau=I\alpha$, $K=\frac12I\omega^2$, $L=I\omega$, $v=R\omega$ | check dimensions term by term |
| Limiting case | simplify one parameter | result should match intuition |

## Worked example 1: Direct calculation

**Problem.** A wheel starts from rest with $\alpha=3.0\,\mathrm{rad/s^2}$ for $4.0\,\mathrm{s}$.
...
$$
$\omega=\alpha t=12\,\mathrm{rad/s}$ and $\Delta\theta=\frac12\alpha t^2=24\,\mathrm{rad}$.
$$
...
**Problem.** A solid cylinder rolls down height $1.2\,\mathrm{m}$.
...
$$
$Mgh=\frac12Mv^2+\frac12(\frac12MR^2)(v/R)^2$, so $v=\sqrt{4gh/3}=3.96\,\mathrm{m/s}$.
$$
...
- Leaving out rotational kinetic energy.
- Using torque without specifying an axis.
- Applying rolling constraints when slipping occurs.
