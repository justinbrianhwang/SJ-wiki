---
title: Ampere, Faraday, Lenz, and Inductance
sidebar_position: 21
---

# Ampere, Faraday, Lenz, and Inductance

Ampere, Faraday, Lenz, and Inductance covers magnetic fields from currents, magnetic flux, induction, Lenz's law, and inductors. The goal is to recognize the physical structure before choosing equations. In the HRW progression, this topic extends earlier mechanics and prepares later field, wave, or modern-physics reasoning by reusing the same habits: define the system, choose signs and variables, state assumptions, and check dimensions.
...
- **Primary symbols:** $\mu_0$, $I$, $\Phi_B$, $\mathcal E$, $L$.
- **Reference equations:** $\oint\vec B\cdot d\vec l=\mu_0I_{enc}$, $B=\mu_0I/(2\pi r)$, $\mathcal E=-N d\Phi_B/dt$, $U_L=\frac12LI^2$.
...
$$
\begin{aligned}
\text{core formulas} &: $\oint\vec B\cdot d\vec l=\mu_0I_{enc}$, $B=\mu_0I/(2\pi r)$, $\mathcal E=-N d\Phi_B/dt$, $U_L=\frac12LI^2$.
\end{aligned}
$$
...
|---|---|---|
| Main model | magnetic fields from currents, magnetic flux, induction, Lenz's law, and inductors | assumptions must match the formula |
| Symbols | $\mu_0$, $I$, $\Phi_B$, $\mathcal E$, $L$ | define each before use |
| Key formulas | $\oint\vec B\cdot d\vec l=\mu_0I_{enc}$, $B=\mu_0I/(2\pi r)$, $\mathcal E=-N d\Phi_B/dt$, $U_L=\frac12LI^2$ | check dimensions term by term |
| Limiting case | simplify one parameter | result should match intuition |

## Worked example 1: Direct calculation

**Problem.** A long straight wire carries $12\,\mathrm{A}$. Find $B$ at $0.050\,\mathrm{m}$.
...
$$
$B=\mu_0I/(2\pi r)=(4\pi\times10^{-7})(12)/(2\pi0.050)=4.8\times10^{-5}\,\mathrm{T}$..
$$
...
**Problem.** A $200$-turn coil of area $0.010\,\mathrm{m^2}$ changes from $0.20\,\mathrm{T}$ to $0.050\,\mathrm{T}$ in $0.30\,\mathrm{s}$. Find emf magnitude.
...
$$
$\vert \mathcal E\vert =N A\vert \Delta B\vert /\Delta t=200(0.010)(0.150)/0.30=1.0\,\mathrm{V}$..
$$
...
import math
mu0=4*math.pi*1e-7
print(mu0*12/(2*math.pi*0.050),200*0.010*0.150/0.30)
```

## Common pitfalls

- Using Ampere's law without enough symmetry.
- Forgetting angle in magnetic flux.
- Predicting induced current that helps the flux change.
