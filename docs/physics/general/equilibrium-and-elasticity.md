...
Equilibrium and Elasticity covers static equilibrium, torque balance, stress, strain, and elastic moduli. The goal is to recognize the physical structure before choosing equations. In the HRW progression, this topic extends earlier mechanics and prepares later field, wave, or modern-physics reasoning by reusing the same habits: define the system, choose signs and variables, state assumptions, and check dimensions.
...
- **Primary symbols:** $\sum\vec F=0$, $\sum\tau=0$, stress, strain, $Y$.
- **Reference equations:** $\sum F_x=0$, $\sum F_y=0$, $\sum\tau=0$, $\Delta L=FL/(AY)$.
...
$$
\begin{aligned}
\text{core formulas} &: $\sum F_x=0$, $\sum F_y=0$, $\sum\tau=0$, $\Delta L=FL/(AY)$.
\end{aligned}
$$
...
|---|---|---|
| Main model | static equilibrium, torque balance, stress, strain, and elastic moduli | assumptions must match the formula |
| Symbols | $\sum\vec F=0$, $\sum\tau=0$, stress, strain, $Y$ | define each before use |
| Key formulas | $\sum F_x=0$, $\sum F_y=0$, $\sum\tau=0$, $\Delta L=FL/(AY)$ | check dimensions term by term |
| Limiting case | simplify one parameter | result should match intuition |

## Worked example 1: Direct calculation

**Problem.** A $4.0\,\mathrm{m}$ beam of weight $120\,\mathrm{N}$ holds a $200\,\mathrm{N}$ sign at its end; a cable at the end is $30^\circ$ above the beam.
...
$$
Torque about the hinge gives $(T\sin30^\circ)(4.0)=120(2.0)+200(4.0)$, so $T=520\,\mathrm{N}$..
$$
...
**Problem.** A steel wire has $L=2.0\,\mathrm{m}$, $A=1.0\times10^{-6}\,\mathrm{m^2}$, $Y=2.0\times10^{11}\,\mathrm{Pa}$, and load $50\,\mathrm{N}$.
...
$$
$\Delta L=FL/(AY)=50(2.0)/[(1.0\times10^{-6})(2.0\times10^{11})]=5.0\times10^{-4}\,\mathrm{m}$.
$$
...
```python
import math
T=(120*2+200*4)/(4*math.sin(math.radians(30)))
dL=50*2/(1e-6*2e11)
print(T,dL)
```
...
- Forgetting torque balance.
- Choosing an inconvenient torque axis.
- Using force instead of perpendicular lever arm.
- Applying linear elasticity beyond the elastic limit.
