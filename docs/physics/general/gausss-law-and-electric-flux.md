...
Gauss's Law and Electric Flux covers electric flux, Gaussian surfaces, symmetry, and conductors. The goal is to recognize the physical structure before choosing equations. In the HRW progression, this topic extends earlier mechanics and prepares later field, wave, or modern-physics reasoning by reusing the same habits: define the system, choose signs and variables, state assumptions, and check dimensions.
...
- **Primary symbols:** $\Phi_E$, $\vec E$, $d\vec A$, $q_{enc}$, $\epsilon_0$.
- **Reference equations:** $\Phi_E=\int\vec E\cdot d\vec A$, $\oint\vec E\cdot d\vec A=q_{enc}/\epsilon_0$, $E_{line}=\lambda/(2\pi\epsilon_0r)$.
...
$$
\begin{aligned}
\text{core formulas} &: $\Phi_E=\int\vec E\cdot d\vec A$, $\oint\vec E\cdot d\vec A=q_{enc}/\epsilon_0$, $E_{line}=\lambda/(2\pi\epsilon_0r)$.
\end{aligned}
$$
...
|---|---|---|
| Main model | electric flux, Gaussian surfaces, symmetry, and conductors | assumptions must match the formula |
| Symbols | $\Phi_E$, $\vec E$, $d\vec A$, $q_{enc}$, $\epsilon_0$ | define each before use |
| Key formulas | $\Phi_E=\int\vec E\cdot d\vec A$, $\oint\vec E\cdot d\vec A=q_{enc}/\epsilon_0$, $E_{line}=\lambda/(2\pi\epsilon_0r)$ | check dimensions term by term |
| Limiting case | simplify one parameter | result should match intuition |

## Worked example 1: Direct calculation

**Problem.** A very long line has $\lambda=3.0\times10^{-6}\,\mathrm{C/m}$. Find $E$ at $0.20\,\mathrm{m}$.
...
$$
$E=\lambda/(2\pi\epsilon_0r)=3.0\times10^{-6}/[2\pi(8.85\times10^{-12})(0.20)]=2.70\times10^5\,\mathrm{N/C}$..
$$
...
**Problem.** A closed surface encloses $+5.0\,\mathrm{nC}$. Find total flux.
...
$$
$\Phi_E=q_{enc}/\epsilon_0=(5.0\times10^{-9})/(8.85\times10^{-12})=5.65\times10^2\,\mathrm{N\,m^2/C}$..
$$
...
import math
eps0=8.85e-12
print(3e-6/(2*math.pi*eps0*0.20),5e-9/eps0)
```

## Common pitfalls

- Trying to extract $E$ without symmetry.
- Counting charges outside the Gaussian surface.
- Forgetting outward area direction.
- Confusing zero net flux with zero field.
...
- [Coulomb's Law and Electric Fields](/physics/general/coulombs-law-and-electric-fields)
