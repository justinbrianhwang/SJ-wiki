---
title: "Quantum Intro: Photons and Matter Waves"
sidebar_position: 26
---

# Quantum Intro: Photons and Matter Waves

Quantum Intro: Photons and Matter Waves covers photons, photoelectric effect, matter waves, uncertainty, and quantization. The goal is to recognize the physical structure before choosing equations. In the HRW progression, this topic extends earlier mechanics and prepares later field, wave, or modern-physics reasoning by reusing the same habits: define the system, choose signs and variables, state assumptions, and check dimensions.
...
- **Primary symbols:** $h$, $f$, $\lambda$, $p$, $\phi$, $K_{max}$.
- **Reference equations:** $E=hf=hc/\lambda$, $p=h/\lambda$, $K_{max}=hf-\phi$, $\lambda=h/p$, $\Delta x\Delta p\ge\hbar/2$.
...
$$
\begin{aligned}
\text{core formulas} &: $E=hf=hc/\lambda$, $p=h/\lambda$, $K_{max}=hf-\phi$, $\lambda=h/p$, $\Delta x\Delta p\ge\hbar/2$.
\end{aligned}
$$
...
|---|---|---|
| Main model | photons, photoelectric effect, matter waves, uncertainty, and quantization | assumptions must match the formula |
| Symbols | $h$, $f$, $\lambda$, $p$, $\phi$, $K_{max}$ | define each before use |
| Key formulas | $E=hf=hc/\lambda$, $p=h/\lambda$, $K_{max}=hf-\phi$, $\lambda=h/p$, $\Delta x\Delta p\ge\hbar/2$ | check dimensions term by term |
| Limiting case | simplify one parameter | result should match intuition |

## Worked example 1: Direct calculation

**Problem.** Find energy of a $500\,\mathrm{nm}$ photon using $hc=1240\,\mathrm{eV\,nm}$.
...
$$
$E=hc/\lambda=1240/500=2.48\,\mathrm{eV}=3.97\times10^{-19}\,\mathrm{J}$..
$$
...
**Problem.** Find the de Broglie wavelength of an electron moving at $1.0\times10^6\,\mathrm{m/s}$.
...
$$
$p=mv=(9.11\times10^{-31})(1.0\times10^6)=9.11\times10^{-25}\,\mathrm{kg\,m/s}$, so $\lambda=h/p=7.27\times10^{-10}\,\mathrm{m}=0.727\,\mathrm{nm}$..
$$
...
h=6.626e-34
E_eV=1240/500
lam=h/(9.11e-31*1.0e6)
print(E_eV,E_eV*1.60e-19,lam)
```

## Common pitfalls

- Thinking brightness sets photoelectron maximum energy.
- Using the $1240\,\mathrm{eV\,nm}$ shortcut with meters.
- Applying nonrelativistic momentum near $c$.
- Interpreting matter waves as ordinary material ripples.
