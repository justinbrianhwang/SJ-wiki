---
title: First Law, Entropy, and Second Law
sidebar_position: 15
---

# First Law, Entropy, and Second Law

First Law, Entropy, and Second Law covers thermodynamic energy accounting, gas work, entropy, engines, and irreversibility. The goal is to recognize the physical structure before choosing equations. In the HRW progression, this topic extends earlier mechanics and prepares later field, wave, or modern-physics reasoning by reusing the same habits: define the system, choose signs and variables, state assumptions, and check dimensions.
...
- **Primary symbols:** $Q$, $W$, $E_{int}$, $S$, $T_H$, $T_C$.
- **Reference equations:** $\Delta E_{int}=Q-W$, $W=\int P\,dV$, $dS=dQ_{rev}/T$, $e_{Carnot}=1-T_C/T_H$.
...
$$
\begin{aligned}
\text{core formulas} &: $\Delta E_{int}=Q-W$, $W=\int P\,dV$, $dS=dQ_{rev}/T$, $e_{Carnot}=1-T_C/T_H$.
\end{aligned}
$$
...
|---|---|---|
| Main model | thermodynamic energy accounting, gas work, entropy, engines, and irreversibility | assumptions must match the formula |
| Symbols | $Q$, $W$, $E_{int}$, $S$, $T_H$, $T_C$ | define each before use |
| Key formulas | $\Delta E_{int}=Q-W$, $W=\int P\,dV$, $dS=dQ_{rev}/T$, $e_{Carnot}=1-T_C/T_H$ | check dimensions term by term |
| Limiting case | simplify one parameter | result should match intuition |

## Worked example 1: Direct calculation

**Problem.** A gas absorbs $500\,\mathrm{J}$ of heat and does $180\,\mathrm{J}$ of work. Find $\Delta E_{int}$.
...
$$
With work done by the system, $\Delta E_{int}=Q-W=500-180=320\,\mathrm{J}$..
$$
...
**Problem.** A heat engine runs between $600\,\mathrm{K}$ and $300\,\mathrm{K}$ and absorbs $2000\,\mathrm{J}$. Find maximum efficiency and work.
...
$$
$e=1-T_C/T_H=1-300/600=0.50$, so $W=eQ_H=1000\,\mathrm{J}$..
$$
...
- Thinking entropy cannot decrease locally.
