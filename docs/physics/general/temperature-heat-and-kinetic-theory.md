---
title: Temperature, Heat, and Kinetic Theory
sidebar_position: 14
---

# Temperature, Heat, and Kinetic Theory

Temperature, Heat, and Kinetic Theory covers temperature, heat transfer, phase change, ideal gases, and molecular kinetic energy. The goal is to recognize the physical structure before choosing equations. In the HRW progression, this topic extends earlier mechanics and prepares later field, wave, or modern-physics reasoning by reusing the same habits: define the system, choose signs and variables, state assumptions, and check dimensions.
...
- **Primary symbols:** $T$, $Q$, $c$, $L$, $P$, $V$, $n$.
- **Reference equations:** $Q=mc\Delta T$, $Q=mL$, $PV=nRT$, $K_{avg}=\frac32k_BT$, $E_{int}=\frac32nRT$.
...
$$
\begin{aligned}
\text{core formulas} &: $Q=mc\Delta T$, $Q=mL$, $PV=nRT$, $K_{avg}=\frac32k_BT$, $E_{int}=\frac32nRT$.
\end{aligned}
$$
...
|---|---|---|
| Main model | temperature, heat transfer, phase change, ideal gases, and molecular kinetic energy | assumptions must match the formula |
| Symbols | $T$, $Q$, $c$, $L$, $P$, $V$, $n$ | define each before use |
| Key formulas | $Q=mc\Delta T$, $Q=mL$, $PV=nRT$, $K_{avg}=\frac32k_BT$, $E_{int}=\frac32nRT$ | check dimensions term by term |
| Limiting case | simplify one parameter | result should match intuition |

## Worked example 1: Direct calculation

**Problem.** Heat $0.50\,\mathrm{kg}$ of water from $20^\circ\mathrm{C}$ to $80^\circ\mathrm{C}$ using $c=4186\,\mathrm{J/(kg\,K)}$.
...
$$
$\Delta T=60\,\mathrm{K}$, so $Q=(0.50)(4186)(60)=1.26\times10^5\,\mathrm{J}$..
$$
...
**Problem.** An ideal gas goes from $P_1=1.0\,\mathrm{atm}$, $V_1=3.0\,\mathrm{L}$, $T_1=300\,\mathrm{K}$ to $V_2=1.5\,\mathrm{L}$, $T_2=360\,\mathrm{K}$. Find $P_2$.
...
$$
$P_2=P_1(V_1/V_2)(T_2/T_1)=1.0(3.0/1.5)(360/300)=2.4\,\mathrm{atm}$..
$$
...
```python
Q=0.50*4186*(80-20)
P2=1.0*(3.0/1.5)*(360/300)
print(Q,P2)
```

## Common pitfalls

- Treating heat as stored in a body.
- Using Celsius in ideal-gas ratios.
- Forgetting phase-change heat.
