...
Relativity Intro covers special relativity, Lorentz factor, time dilation, length contraction, relativistic energy, and velocity addition. The goal is to recognize the physical structure before choosing equations. In the HRW progression, this topic extends earlier mechanics and prepares later field, wave, or modern-physics reasoning by reusing the same habits: define the system, choose signs and variables, state assumptions, and check dimensions.
...
- **Primary symbols:** $c$, $v$, $\gamma$, $\Delta t_0$, $L_0$, $p$, $E$.
- **Reference equations:** $\gamma=1/\sqrt{1-v^2/c^2}$, $\Delta t=\gamma\Delta t_0$, $L=L_0/\gamma$, $K=(\gamma-1)mc^2$.
...
$$
\begin{aligned}
\text{core formulas} &: $\gamma=1/\sqrt{1-v^2/c^2}$, $\Delta t=\gamma\Delta t_0$, $L=L_0/\gamma$, $K=(\gamma-1)mc^2$.
\end{aligned}
$$
...
|---|---|---|
| Main model | special relativity, Lorentz factor, time dilation, length contraction, relativistic energy, and velocity addition | assumptions must match the formula |
| Symbols | $c$, $v$, $\gamma$, $\Delta t_0$, $L_0$, $p$, $E$ | define each before use |
| Key formulas | $\gamma=1/\sqrt{1-v^2/c^2}$, $\Delta t=\gamma\Delta t_0$, $L=L_0/\gamma$, $K=(\gamma-1)mc^2$ | check dimensions term by term |
| Limiting case | simplify one parameter | result should match intuition |

## Worked example 1: Direct calculation

**Problem.** A muon with proper lifetime $2.2\,\mu\mathrm{s}$ moves at $0.98c$. Find lab lifetime.
...
$$
$\gamma=1/\sqrt{1-0.98^2}=5.03$, so $\Delta t=5.03(2.2)=11.1\,\mu\mathrm{s}$..
$$
...
**Problem.** Find kinetic energy of an electron moving at $0.80c$ in units of $mc^2$.
...
$$
$\gamma=1/\sqrt{1-0.80^2}=1.667$, so $K=(\gamma-1)mc^2=0.667mc^2$..
$$
...
```python
import math
g1=1/math.sqrt(1-0.98**2)
g2=1/math.sqrt(1-0.80**2)
print(g1,2.2*g1,g2-1)
```

## Common pitfalls

- Using time dilation without identifying proper time.
- Contracting lengths perpendicular to motion.
- Adding high speeds by ordinary arithmetic.
- Forgetting Newtonian mechanics is the low-speed limit.
