---
title: "Oscillations: SHM, Damping, and Resonance"
sidebar_position: 12
---

# Oscillations: SHM, Damping, and Resonance

Oscillations: SHM, Damping, and Resonance covers simple harmonic motion, energy exchange, damping, driving, and resonance. The goal is to recognize the physical structure before choosing equations. In the HRW progression, this topic extends earlier mechanics and prepares later field, wave, or modern-physics reasoning by reusing the same habits: define the system, choose signs and variables, state assumptions, and check dimensions.
...
- **Primary symbols:** $A$, $\omega$, $T$, $f$, $k$, $m$, $L$.
- **Reference equations:** $a=-\omega^2x$, $x=A\cos(\omega t+\phi)$, $T=2\pi\sqrt{m/k}$, $T=2\pi\sqrt{L/g}$, $E=\frac12kA^2$.
...
$$
\begin{aligned}
\text{core formulas} &: $a=-\omega^2x$, $x=A\cos(\omega t+\phi)$, $T=2\pi\sqrt{m/k}$, $T=2\pi\sqrt{L/g}$, $E=\frac12kA^2$.
\end{aligned}
$$
...
|---|---|---|
| Main model | simple harmonic motion, energy exchange, damping, driving, and resonance | assumptions must match the formula |
| Symbols | $A$, $\omega$, $T$, $f$, $k$, $m$, $L$ | define each before use |
| Key formulas | $a=-\omega^2x$, $x=A\cos(\omega t+\phi)$, $T=2\pi\sqrt{m/k}$, $T=2\pi\sqrt{L/g}$, $E=\frac12kA^2$ | check dimensions term by term |
| Limiting case | simplify one parameter | result should match intuition |

## Worked example 1: Direct calculation

**Problem.** A $0.40\,\mathrm{kg}$ mass on a $160\,\mathrm{N/m}$ spring oscillates with amplitude $0.050\,\mathrm{m}$. Find period and maximum speed.
...
$$
$\omega=\sqrt{k/m}=20.0\,\mathrm{rad/s}$, $T=2\pi/\omega=0.314\,\mathrm{s}$, and $v_{max}=A\omega=1.00\,\mathrm{m/s}$..
$$
...
**Problem.** What pendulum length gives $T=1.50\,\mathrm{s}$ near Earth?
...
$$
$L=g(T/2\pi)^2=9.8(1.50/2\pi)^2=0.558\,\mathrm{m}$..
$$
...
```python
import math
omega=math.sqrt(160/0.40)
print(2*math.pi/omega,0.050*omega,9.8*(1.50/(2*math.pi))**2)
```
...
- Ignoring damping in real resonance.
