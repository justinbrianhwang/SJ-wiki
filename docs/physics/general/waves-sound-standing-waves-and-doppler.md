---
title: Waves, Sound, Standing Waves, and Doppler Effect
sidebar_position: 13
---

# Waves, Sound, Standing Waves, and Doppler Effect

Waves, Sound, Standing Waves, and Doppler Effect covers mechanical waves, sound, standing-wave modes, and Doppler shifts. The goal is to recognize the physical structure before choosing equations. In the HRW progression, this topic extends earlier mechanics and prepares later field, wave, or modern-physics reasoning by reusing the same habits: define the system, choose signs and variables, state assumptions, and check dimensions.
...
- **Primary symbols:** $v$, $f$, $\lambda$, $k$, $\omega$, intensity.
- **Reference equations:** $v=f\lambda$, $y=A\sin(kx-\omega t)$, $v=\sqrt{F_T/\mu}$, $f_n=nv/(2L)$, $f'=f(v\pm v_o)/(v\mp v_s)$.
...
$$
\begin{aligned}
\text{core formulas} &: $v=f\lambda$, $y=A\sin(kx-\omega t)$, $v=\sqrt{F_T/\mu}$, $f_n=nv/(2L)$, $f'=f(v\pm v_o)/(v\mp v_s)$.
\end{aligned}
$$
...
|---|---|---|
| Main model | mechanical waves, sound, standing-wave modes, and Doppler shifts | assumptions must match the formula |
| Symbols | $v$, $f$, $\lambda$, $k$, $\omega$, intensity | define each before use |
| Key formulas | $v=f\lambda$, $y=A\sin(kx-\omega t)$, $v=\sqrt{F_T/\mu}$, $f_n=nv/(2L)$, $f'=f(v\pm v_o)/(v\mp v_s)$ | check dimensions term by term |
| Limiting case | simplify one parameter | result should match intuition |

## Worked example 1: Direct calculation

**Problem.** A string has tension $80\,\mathrm{N}$ and linear density $0.020\,\mathrm{kg/m}$. Find wave speed and wavelength at $50\,\mathrm{Hz}$.
...
$$
$v=\sqrt{80/0.020}=63.2\,\mathrm{m/s}$ and $\lambda=v/f=63.2/50=1.26\,\mathrm{m}$..
$$
...
**Problem.** A $700\,\mathrm{Hz}$ siren approaches a stationary observer at $25\,\mathrm{m/s}$. Use sound speed $343\,\mathrm{m/s}$.
...
$$
$f'=f v/(v-v_s)=700(343)/(343-25)=755\,\mathrm{Hz}$..
$$
...
```python
import math
v=math.sqrt(80/0.020)
print(v,v/50,700*343/(343-25))
```

## Common pitfalls

- Confusing medium motion with wave propagation.
- Using $\omega$ where $f$ belongs.
- Using fixed-string harmonics for closed-open pipes.
- Choosing Doppler signs by memorization rather than physical increase/decrease.
